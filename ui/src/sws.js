...
import SWSLayout from './swsLayout';
...
;(function($, window, document, undefined) {
  ...
  const pluginName = 'swaggerstatsui';
  ...
  const SWSUI = function(element, options) {
  ...
  SWSUI.prototype.render = function() {
    ...
    for (let pageId in this.layout.pages) {
      ...
      const elemPageContent = $('<div id="' + pageId + '_content"></div>');
      ...
      // Page rows
      for (let rowId in page.rows) {
        const row = page.rows[rowId];
        row.id = pageId + '_' + rowId;
        const rowextraclass = ('class' in row ? row.class : '');
        const elemRow = $('<div id="' + row.id + '" class="row ' + rowextraclass + '">');
        elemPageContent.append(elemRow);

        // Row Columns
        for (let colId in row.columns) {
          const col = row.columns[colId];
          col.id = colId;
          const elemCol = $('<div id="' + col.id + '" class="' + col.class + '"></div>');
    ...
    const uxHtml = '<li class="sws-tool-li"><a href="./ux" style="margin-top: 8px;">' +
      '<span class="label label-success" style="font-size: 16px">Try New UX</span></a></li>';
    const uxNav = $(uxHtml);
  ...
  SWSUI.prototype.buildNavControls = function() {
    ...
    const elemNavCtrls = $('.sws-nav-ctrls');

    const elemRefresh = $('<div class="sws-refresh-group"></div>');
    ...
    const elemTime = $('<div class="sws-time-group"></div>');
  ...
  SWSUI.prototype.renderCol = function(page, row, col, elemCol) {
    ...
    switch (col.type) {
      ...
      case 'chart':
        ...
        break;
      case 'datatable':
        const tid = elemCol.attr('id') + '_tbl';
        col.options.id = tid;
        col.options.swsId = this.elementId;
        const args = { dataTableSettings: col.dataTableSettings };
  ...
  SWSUI.prototype.enableNavigation = function() {
    ...
    $(window).on('hashchange', function(e) {
      ...
      const loginLocHash = '#' + that.layout.loginpage;
      if (that.loggingIn) {
    ...
    if (hashLoc !== '') {
      ...
      const startLocHash = '#' + this.layout.startpage;
  ...
  SWSUI.prototype.setActive = function(pageIdHash) {
    ...
    const hasharray = pageIdHash.split('=', 2);
  ...
  SWSUI.prototype.refreshStats = function() {
    ...
    this.startProgress();
    const activeDef = this.layout.pages[this.activePageId];
    const getdataDef = activeDef.getdata;
    let getdataURL = getdataDef.url;
    if (this.swsBasePath !== null) {
      getdataURL = this.swsBasePath + '/' + getdataDef.url;
    }
    const getdataReq = { type: getdataDef.type, url: getdataURL, data: {} };
    ...
    if (('data' in getdataDef) && ('fields' in getdataDef.data) && (getdataDef.data.fields instanceof Array)) {
      for (let k = 0; k < getdataDef.data.fields.length; k++) {
    ...
    if ('getfieldsonce' in activeDef) {
      for (let i = 0; i < activeDef.getfieldsonce.length; i++) {
    ...
    const that = this;
    const jqxhr = $.ajax(getdataReq)
      .done(function(msg) {

        // Login attempt successfull
        const xswsauth = jqxhr.getResponseHeader('x-sws-authenticated');
      ...
      .fail(function(jqXHR, textStatus) {
        ...
        if (jqXHR.status === 403) {
          ...
          const locHash = '#' + that.layout.loginpage;
  ...
  SWSUI.prototype.processStatsData = function(getdataDef, data) {
    ...
    if ('timeline' in data) {
      ...
      if (this.apistats && this.apistats.timeline && this.apistats.timeline.data) {
        for (let key in this.apistats.timeline.data) {
          const entry = this.apistats.timeline.data[key];
          entry.tc = parseInt(key);
          const ts = entry.tc * this.apistats.timeline.settings.bucket_duration;
  ...
  SWSUI.prototype.onLogout = function(Event) {
    ...
    let getdataURL = 'logout';
    if (this.swsBasePath !== null) {
      getdataURL = this.swsBasePath + '/logout';
    }
    const getdataReq = { type: 'GET', url: getdataURL, data: {} };

    const that = this;
    $.ajax(getdataReq)
      .done(function(msg) {
        const startLocHash = '#' + that.layout.startpage;
  ...
  SWSUI.prototype.onRefreshClick = function(Event) {
    ...
    const interval = parseInt($(Event.target).attr('interval'));
    if (interval == 0) {
      // Refresh immediately
      this.refreshStats();
      return;
  ...
  SWSUI.prototype.onChangeAPIOp = function(srcEvent) {
    ...
    const locHash = '#sws_apiop=' + selectedOp.method + ',' + selectedOp.path;
  ...
  SWSUI.prototype.getLatestTimelineValue = function(key, prop) {
    ...
    try {
      const last = this.apistats.timeline.data[this.apistats.timeline.settings.bucket_current];
  ...
  SWSUI.prototype.getTimelineTrend = function(key, prop) {
    ...
    const n = this.timeline_array.length;
    let sum_x = 0;
    let sum_y = 0;
    let sum_xy = 0;
    let sum_xx = 0;
    let sum_yy = 0;
    for (let i = 0; i < n; i++) {
      const entry = this.timeline_array[i];
      const x = ((key in entry) && (prop in entry[key]) ? entry[key][prop] : 0);
      sum_x += x;
      sum_y += i;
      sum_xy += (x * i);
      sum_xx += (x * x);
    ...
    let slope = 0;
    const divby = (n * sum_xx - sum_x * sum_x);
    if (divby != 0) {
    ...
    const res = (slope > 0 ? 'up' : (slope < 0 ? 'down' : ''));
    return res;
  };
  ...
  SWSUI.prototype.buildTimeSeriesChartData = function(chartdata, key, datasets) {
    ...
    const start_label = this.timeline_array[0].timelabel;
    while ((chartdata.labels.length > 0)
    && (chartdata.labels[0] != start_label)) {
      chartdata.labels.shift();
      for (let ks = 0; ks < datasets.length; ks++) {
    ...
    for (j = 0; j < chartdata.labels.length; j++) {
      for (let ku = 0; ku < datasets.length; ku++) {
    ...
    for (; j < this.timeline_array.length; j++) {
      ...
      for (let ka = 0; ka < datasets.length; ka++) {
  ...
  SWSUI.prototype.updateByMethodChartData = function(chartdata, prop, adjust) {
    const vGetter = (typeof prop === 'function') ? prop : null;
    adjust = (typeof adjust === 'function') ? adjust : null;
    for (let method in this.apistats.method) {
      const reqStats = this.apistats.method[method];
      let idx = chartdata.labels.indexOf(method);
      let val = 0;
      if (vGetter) {
        const vRaw = vGetter(reqStats);
  ...
  SWSUI.prototype.formatBytes = function(a, b) {
    ...
    const c = 1e3, d = b || 2, e = ["Bytes", "KB", "MB", "GB", "TB", "PB", "EB", "ZB", "YB"],
  ...
  SWSUI.prototype.updateSummary = function() {
    ...
    // Update Widgets
    $('#sws_summ_wRq').swswidget('setvalue', {
      value: this.apistats.all.requests,
      trend: this.getTimelineTrend('stats', 'requests'),
    });
    //$('#sws_summ_wRp').swswidget('setvalue', { value:(this.apistats.all.requests-this.apistats.all.responses) } );

    $('#sws_summ_wApd').swswidget('setvalue', {
      value: this.apistats.all.apdex_score.toFixed(2), customtrend: function(elemTrend) {
        const apd1 = that.apistats.all.apdex_score.toFixed(2) * 100;
    ...
    $('#sws_summ_wRRte').swswidget('setvalue', {
      value: this.getLatestTimelineValue('stats', 'req_rate').toFixed(4),
      extra: 'req/sec',
      trend: this.getTimelineTrend('stats', 'req_rate'),
    });
    $('#sws_summ_wERte').swswidget('setvalue', {
      value: this.getLatestTimelineValue('stats', 'err_rate').toFixed(4),
      extra: 'err/sec',
      trend: this.getTimelineTrend('stats', 'err_rate'),
    });
    $('#sws_summ_wAHt').swswidget('setvalue', this.formatWValDurationMS({ value: this.apistats.all.avg_time }));

    $('#sws_summ_wCpu').swswidget('setvalue', {
      value: this.apistats.sys.cpu.toFixed(2) + ' %', customtrend: function(elemTrend) {
        const cpu1 = that.apistats.sys.cpu, cpu2 = 100 - that.apistats.sys.cpu;
    ...
    $('#sws_summ_wErr').swswidget('setvalue', {
      value: this.apistats.all.errors,
      total: this.apistats.all.requests,
      trend: this.getTimelineTrend('stats', 'errors'),
    });
    $('#sws_summ_wSs').swswidget('setvalue', {
      value: this.apistats.all.success,
      total: this.apistats.all.requests,
      trend: this.getTimelineTrend('stats', 'success'),
    });
    $('#sws_summ_wRed').swswidget('setvalue', {
      value: this.apistats.all.redirect,
      total: this.apistats.all.requests,
      trend: this.getTimelineTrend('stats', 'redirect'),
    });
    $('#sws_summ_wCe').swswidget('setvalue', {
      value: this.apistats.all.client_error,
      total: this.apistats.all.requests,
      trend: this.getTimelineTrend('stats', 'client_error'),
    });
    $('#sws_summ_wSe').swswidget('setvalue', {
      value: this.apistats.all.server_error,
      total: this.apistats.all.requests,
      trend: this.getTimelineTrend('stats', 'server_error'),
    });
    ...
    // Update CPU chart
    const elemCPUChart = $('#sws_summ_cCpu');
    ...
    */
    ...
    const elemTimelineChart = $('#sws_summ_cTl');
  ...
  SWSUI.prototype.updateRequests = function() {
    ...
    // Table
    const elemRbyMTable = $('#sws_req_tRbM');
    elemRbyMTable.swstable('clear');
    for (let method in this.apistats.method) {
      var reqStats = this.apistats.method[method];
      const row = [method, reqStats.requests, reqStats.responses, (reqStats.requests - reqStats.responses),
    ...
    // Update charts
    const elemRbyMChart = $('#sws_req_cRbM');
    ...
    const elemEbMChart = $('#sws_req_cEbM');
    this.updateByMethodChartData(elemEbMChart.swschart('getchartdata'), 'errors');
    elemEbMChart.swschart('update');

    // Update Avg Time chart
    const elemRTimeChart = $('#sws_req_cRTime');
    this.updateByMethodChartData(elemRTimeChart.swschart('getchartdata'), 'avg_time', function(val) {
      return val.toFixed(4);
    });
    elemRTimeChart.swschart('update');

    // Update Requests in processing chart
    const elemRProcChart = $('#sws_req_cRProc');
    this.updateByMethodChartData(elemRProcChart.swschart('getchartdata'), function(reqStats) {
      return (reqStats.requests - reqStats.responses);
    });
    elemRProcChart.swschart('update');

    // Update Requests Rate chart
    const elemRRateChart = $('#sws_req_cRRbM');
    this.updateByMethodChartData(elemRRateChart.swschart('getchartdata'), 'req_rate', function(val) {
      return val.toFixed(4);
    });
    elemRRateChart.swschart('update');

    // Update Error Rate chart
    const elemERateChart = $('#sws_req_cERbM');
    this.updateByMethodChartData(elemERateChart.swschart('getchartdata'), 'err_rate', function(val) {
      return val.toFixed(4);
    });
  ...
  SWSUI.prototype.updateErrors = function() {
    ...
    // Table
    const elemErrTable = $('#sws_err_tCode');
    elemErrTable.swstable('clear');

    // Chart
    const elemErrByCodeChart = $('#sws_err_cCode');
    const chartData = elemErrByCodeChart.swschart('getchartdata');

    if (this.apistats.errors.statuscode) {
      for (let statusCode in this.apistats.errors.statuscode) {

        const codeCount = this.apistats.errors.statuscode[statusCode];
        ...
        // Update Chart Data
        const idx = chartData.labels.indexOf(statusCode);
        if (idx != -1) {
    ...
    // Top not found table
    const elem404Table = $('#sws_err_t404');
    elem404Table.swstable('clear');
    if (this.apistats.errors.topnotfound) {
      for (let notFoundPath in this.apistats.errors.topnotfound) {
    ...
    const elem500Table = $('#sws_err_t500');
    elem500Table.swstable('clear');
    if (this.apistats.errors.topservererror) {
      for (let serverErrorPath in this.apistats.errors.topservererror) {
  ...
  SWSUI.prototype.updateLastErrors = function() {
    ...
    const elemErrTable = $('#sws_lerr_tErr');
    elemErrTable.swstable('clear');
    if (this.apistats.lasterrors && this.apistats.lasterrors.length > 0) {
      for (let i = 0; i < this.apistats.lasterrors.length; i++) {
        const errorInfo = this.apistats.lasterrors[i];
        const row = ['', moment(errorInfo.startts).format(),
          errorInfo.method,
          errorInfo.path,
  ...
  SWSUI.prototype.updateLongestReq = function() {
    ...
    const elemLReqTable = $('#sws_lreq_tReq');
    elemLReqTable.swstable('clear');
    if (this.apistats.longestreq && this.apistats.longestreq.length > 0) {
      for (let i = 0; i < this.apistats.longestreq.length; i++) {
        const reqInfo = this.apistats.longestreq[i];
        const row = ['', moment(reqInfo.startts).format(),
          reqInfo.method,
          reqInfo.path,
  ...
  SWSUI.prototype.updateRates = function() {
    ...
    $('#sws_rates_wApd').swswidget('setvalue', {
      value: this.getLatestTimelineValue('stats', 'apdex_score').toFixed(2), customtrend: function(elemTrend) {
        const apd1 = that.getLatestTimelineValue('stats', 'apdex_score').toFixed(2) * 100;
        elemTrend.append($('<div class="swsbox-trend-container"><span class="pie">' + apd1 + '/100</span></div>'));
        elemTrend.find('.pie').peity("donut", { fill: ["#ff9900", "#e7eaec"], radius: 30, innerRadius: 16 });
      },
    });

    $('#sws_rates_wRqR').swswidget('setvalue', {
      value: this.getLatestTimelineValue('stats', 'req_rate').toFixed(4),
      extra: 'req/sec',
      trend: this.getTimelineTrend('stats', 'req_rate'),
    });
    $('#sws_rates_wErR').swswidget('setvalue', {
      value: this.getLatestTimelineValue('stats', 'err_rate').toFixed(4),
      extra: 'err/sec',
      trend: this.getTimelineTrend('stats', 'err_rate'),
    });
    $('#sws_rates_wMHT').swswidget('setvalue', this.formatWValDurationMS({
      value: this.getLatestTimelineValue('stats', 'max_time'),
      trend: this.getTimelineTrend('stats', 'max_time'),
    }));
    $('#sws_rates_wAHT').swswidget('setvalue', this.formatWValDurationMS({
      value: this.getLatestTimelineValue('stats', 'avg_time'),
      trend: this.getTimelineTrend('stats', 'avg_time'),
    }, 2));
    $('#sws_rates_wSHT').swswidget('setvalue', this.formatWValDurationMS({
      value: this.getLatestTimelineValue('stats', 'total_time'),
      trend: this.getTimelineTrend('stats', 'total_time'),
    }));

    //$('#sws_rates_wOApd').swswidget('setvalue', { value:this.apistats.all.apdex_score.toFixed(2),extra:'' } );
    $('#sws_rates_wOApd').swswidget('setvalue', {
      value: this.apistats.all.apdex_score.toFixed(2), customtrend: function(elemTrend) {
        const apd1 = that.apistats.all.apdex_score.toFixed(2) * 100;
        elemTrend.append($('<div class="swsbox-trend-container"><span class="pie">' + apd1 + '/100</span></div>'));
        elemTrend.find('.pie').peity("donut", { fill: ["#ff9900", "#e7eaec"], radius: 30, innerRadius: 16 });
    ...
    const elemApdexChart = $('#sws_rates_cApd');
    ...
    const elemReqErrRateChart = $('#sws_rates_cRER');
  ...
  SWSUI.prototype.updatePayload = function() {
    ...
    // Update Widgets
    $('#sws_payl_wTRqP').swswidget('setvalue', {
      value: this.apistats.all.total_req_clength,
      trend: this.getTimelineTrend('stats', 'total_req_clength'),
    });
    $('#sws_payl_wMRqP').swswidget('setvalue', {
      value: this.apistats.all.max_req_clength,
      trend: this.getTimelineTrend('stats', 'max_req_clength'),
    });
    $('#sws_payl_wARqP').swswidget('setvalue', {
      value: this.apistats.all.avg_req_clength.toFixed(0),
      trend: this.getTimelineTrend('stats', 'avg_req_clength'),
    });
    $('#sws_payl_wTRsP').swswidget('setvalue', {
      value: this.apistats.all.total_res_clength,
      trend: this.getTimelineTrend('stats', 'total_res_clength'),
    });
    $('#sws_payl_wMRsP').swswidget('setvalue', {
      value: this.apistats.all.max_res_clength,
      trend: this.getTimelineTrend('stats', 'max_res_clength'),
    });
    $('#sws_payl_wARsP').swswidget('setvalue', {
      value: this.apistats.all.avg_res_clength.toFixed(0),
      trend: this.getTimelineTrend('stats', 'avg_res_clength'),
    });

    // Update timeline charts
    const elemReqPayloadChart = $('#sws_payl_cRqPl');
    ...
    const elemResPayloadChart = $('#sws_payl_cRsPl');
  ...
  SWSUI.prototype.updateAPI = function() {
    ...
    const elemApiTable = $('#sws_api_tApi');
    elemApiTable.swstable('clear');

    // Show data
    for (let path in this.apistats.apistats) {
      const apiPath = this.apistats.apistats[path];
      for (let method in apiPath) {
        const apiOpStats = apiPath[method];
        let apiOpDef = { swagger: false, deprecated: false, operationId: '', summary: '', description: '' };
  ...
  SWSUI.prototype.switchToAPIOp = function(method, path) {
    const locHash = '#sws_apiop=' + method + ',' + path;
    window.location.hash = locHash;
  };
  ...
  SWSUI.prototype.updateAPIOp = function() {
    ...
    const elemSelect = $('#sws_apiop_opsel').swsapiopsel('update', this.apistats);
    ...
    var selectedOp = {};
    const vals = this.activePageContext.split(',', 2);
    ...
    $('#sws_apiop_wPath').swswidget('setvalue', {
      value: '',
      title: selectedOp.method + ' ' + selectedOp.path,
      subtitle: this.getAPIOpInfoMarkup(selectedOp.path, selectedOp.method),
    });
    ...
    $('#sws_apiop_wApd').swswidget('setvalue', {
      value: opStats.apdex_score.toFixed(2), customtrend: function(elemTrend) {
        const apd1 = opStats.apdex_score.toFixed(2) * 100;
        elemTrend.append($('<div class="swsbox-trend-container"><span class="pie">' + apd1 + '/100</span></div>'));
        elemTrend.find('.pie').peity("donut", { fill: ["#ff9900", "#e7eaec"], radius: 30, innerRadius: 16 });
      },
    });
    ...
    if (('duration' in opDetails) && ('buckets' in opDetails.duration) && ('values' in opDetails.duration)) {
      const elemHTChart = $('#sws_apiop_cHTH');
    ...
    if (('req_size' in opDetails) && ('buckets' in opDetails.req_size) && ('values' in opDetails.req_size)) {
      const elemRQChart = $('#sws_apiop_cRqSH');
    ...
    if (('res_size' in opDetails) && ('buckets' in opDetails.res_size) && ('values' in opDetails.res_size)) {
      const elemRSChart = $('#sws_apiop_cRsSH');
  ...
  SWSUI.prototype.updateHistogramChartData = function(chartdata, buckets, values) {
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      const bucket = i < buckets.length ? buckets[i] : 'inf';
      const value = values[i];

      const bucketLabel = prevBucket + '-' + bucket;
      let idx = chartdata.labels.indexOf(bucketLabel);
  ...
  SWSUI.prototype.updateByCodeChartData = function(chartdata, codesdata) {
    ...
    for (let code in codesdata) {

      const codeobj = codesdata[code];
      const bucket = code;
      const value = 'count' in codeobj ? codeobj.count : 0;

      let idx = chartdata.labels.indexOf(bucket);
  ...
  SWSUI.prototype.getAPIOpInfoMarkup = function(path, method) {
    let markup = '';
    if (('apidefs' in this.apistats) && (path in this.apistats.apidefs) && (method in this.apistats.apidefs[path])) {
      const apiOpDef = this.apistats.apidefs[path][method];
      let isSwagger = 'No';
